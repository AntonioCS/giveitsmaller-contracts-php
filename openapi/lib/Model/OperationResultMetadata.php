 ...
 */
class OperationResultMetadata implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'OperationResultMetadata';
     ...
     * Sets watermark_id
     *
     * @param string|null $watermark_id UUIDv7 of the watermark embedded by an `audio_watermark` operation (for later decode/verify). **Display-only / not yet populated** — gated on the audio_watermark Lambda (`EvPl5fkH`, B3) + the watermark registry (B2, deferred). The field + whitelist ship now (additive) so the result_metadata pipeline + read projection land without a later contract bump; absent in practice until B3 is live.
     *
     * @return self
     */
