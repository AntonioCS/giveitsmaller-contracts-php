...
/**
 * ExternalImportCreatedResponse Class Doc Comment
 *
 * @category Class
 ...
 */
class ExternalImportCreatedResponse implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * @var string
     */
    protected static $openAPIModelName = 'ExternalImportCreatedResponse';
    ...
    public function setExternalSourceId($external_source_id)
    {
        ...
        if ((!preg_match("/^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/", ObjectSerializer::toString($external_source_id)))) {
            throw new \InvalidArgumentException("invalid value for \$external_source_id when calling ExternalImportCreatedResponse., must conform to the pattern /^[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.");
        }
     ...
     * Sets provider
     *
     * @param string $provider Server-detected provider classification from the URL (`s3_presigned` / `gcs_signed` / `azure_sas` / `dropbox` / `public_https`). May differ from the caller's `provider_hint` if the hint was wrong; server's value is authoritative.
     *
     * @return self
     */
