...
class ValidationErrorEnvelope implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets error
     *
     * @param string $error Stable error code. Common values: `INVALID_OPTIONS` (generic option/value validation failure), `REQUIRES_REENCODE` (per ticket I16-CONS — `merge.video` with `re_encode_mode: never` and incompatible inputs; caller resolves by switching to `re_encode_mode: auto` or `always`), `VALIDATION_FAILED` (request/query-param validation failure per the project-wide convention — e.g. `GET /api/v2/credits/usage` invalid `limit`/`offset`). SDKs duck-type on this field for typed error-branch helpers.
     *
     * @return self
     */
