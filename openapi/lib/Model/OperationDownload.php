...
class OperationDownload implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets chosen_quality
     *
     * @param int|null $chosen_quality For a `target_size` encode (`encoding_mode: target_size`): the quality the encode-measure loop settled on. Present only for a target_size output; absent otherwise. Optional/additive (rides the shared property bag — not in any oneOf `required`/`not`). The per-output `/downloads` read is the canonical output-properties surface per the ZjVXHkYK narrowing (alongside `size_bytes`); mirrors the worker source `OperationMetrics.chosen_quality`. Pairs with `target_size_met`.
     *
     * @return self
     */
